...
def AddOnbuttons(currentDrink):
    ...
    if (addonButtonsshown == False):
        ...
        for text, mode in SizeOptions:
            Radiobutton(sizeFrame, text=text,variable=v, value=mode).pack(side=LEFT)
        sizeFrame.grid(column=1, row=rowVar)
        rowVar += 1
        ...
        AddToCartButton = Button(AddOnsFrame, text="Add To Cart", padx=200, pady=10,
                                 command=lambda: removeAddOns(currentDrink))
    ...
    if (len(currentDrink.cartList) > 0):
        if (checkoutShowing == False):
            ...
            checkout = Button(CartFrame, text="Checkout", padx=210, pady=10,
                              command=lambda: checkoutDisplay(currentDrink, subtotal))
...
def checkoutDisplay(currentDrink, total):
    ...
    Creditcardbutton = Button(checkoutdialog, text="Credit Card", command=lambda: CreditCardScreen(currentDrink, total),
                              padx=123, pady=10)
    Creditcardbutton.grid(column=0, row=2)
    debitcardbutton = Button(checkoutdialog, text="Debit Card", command=lambda: DebitCard(currentDrink, total),
                             padx=125, pady=10)
    debitcardbutton.grid(column=0, row=3)
    applepaybutton = Button(checkoutdialog, text="Apple Samsung Pay", padx=100, pady=10,
                            command=lambda: applepay(currentDrink))
    ...
    def applepay(currentDrink):
        ...
        message.pack()
        donebutton = Button(applepaywidow, text="Done", width=15, height=5,
                            command=lambda: Done(currentDrink, applepaywidow))
        donebutton.pack()
    ...
    def DebitCard(currentDrink, total):
        ...
        donebutton = Button(DebitCardwindow, text="Done", width=10, height=3,
                            command=lambda: Done(currentDrink, DebitCardwindow))
